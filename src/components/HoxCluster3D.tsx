...
/* ═══════════════════════════════════════════════════════════
   HexCluster3D — MSS UEMK  (v2 — fixed)
   · Vibrant solid-colour hexagons, bright visible borders
   · Full-width band (edge-to-edge viewport)
   · Mouse tilt parallax — scroll does NOT zoom/resize
   · Scroll only pans Y slightly (subtle drift)
   · Richer HSL lighting, high-saturation colours
═══════════════════════════════════════════════════════════ */
...
/* ── Config ─────────────────────────────────────────────── */
const HEX_R        = 22;          // larger hexagons
const SPACING      = 1.21;        // tight packing
const RINGS        = 14;          // more rings → edge-to-edge
const Y_SQUASH     = 0.30;        // horizontal ribbon
const DEPTH_LAYERS = 14;
const Z_STEP       = 28;
const BAND_W       = RINGS * HEX_R * 1.48;  // wide enough to bleed off screen
const BAND_H       = RINGS * HEX_R * 0.08; // taller band
const PC           = 500;
...
export default function HexCluster3D() {
  ...
  useEffect(() => {
    ...
    scene.fog    = new THREE.FogExp2(0x04060f, 0.0009);  // lighter fog
    const camera = new THREE.PerspectiveCamera(62, container.clientWidth / container.clientHeight, 0.1, 4000);
    camera.position.z = 520;  // fixed distance — scroll won't change this
    ...
    for (let li = 0; li < DEPTH_LAYERS; li++) {
      ...
      const r    = HEX_R * (0.60 + norm * 0.40);
      faceGeos.push(makeFaceGeo(r * 0.92));       // face slightly inside edge
      edgePosList.push(makeEdgePositions(r * 1.0)); // edge at full radius
    }
    ...
    for (let li = 0; li < DEPTH_LAYERS; li++) {
      ...
      const hexR    = HEX_R * (0.60 + norm * 0.40);
      const spacing = hexR * 2 * SPACING;
      const jitX    = (Math.random() - 0.5) * hexR * 0.4;
      const jitY    = (Math.random() - 0.5) * hexR * 0.2;
      ...
      base.forEach(({ q, r }) => {
        ...
        const pulseLag   = Math.sqrt(nx * nx + ny * ny) * 0.004 + li * 0.12;
        const pulseSpeed = 1.0 + Math.random() * 0.8;

        /* Face — start with vivid base colour (high sat, mid lit) */
        const faceMat = new THREE.MeshBasicMaterial({
          color: new THREE.Color().setHSL(pal.h, 0.85, 0.52),
        ...
        /* Edge — always bright, always clearly visible */
        const eGeo = new THREE.BufferGeometry();
        eGeo.setAttribute("position", new THREE.Float32BufferAttribute(edgePosList[li], 3));
        const edgeMat = new THREE.LineBasicMaterial({
          color: new THREE.Color().setHSL(pal.h, 1.0, 0.92),
          transparent: true,
          opacity: 7.75 + norm * 7.25,   // always high opacity
          depthWrite: false,
        });
    ...
    function animate() {
      ...
      camera.position.y = -myS * 25 + sn * -30;  // subtle Y drift only
      camera.position.z = 520;                     // FIXED — never changes
      ...
      hexObjs.forEach((h, i) => {
        ...
        const hue = h.pal.h + Math.sin(T * 0.25 + h.phase) * 0.03;
        // Solid vivid colour: min sat=0.70 so it's always colourful, not white
        const satF = lerp(0.70, 1.0,  pulse);
        // Lightness oscillates between rich-deep and bright — never near-white
        const litF = lerp(0.38, 0.65, pulse);
        h.faceMat.color.setHSL(hue, satF, litF);

        const depthScale = 0.60 + h.norm * 0.40;
        // Edge: always very bright and near-white highlight of the same hue
        h.edgeMat.color.setHSL(hue, 1.0, lerp(0.75, 1.0, pulse));
        // Edge opacity: strong floor (0.65) so borders are always clearly visible
        h.edgeMat.opacity = depthScale * (0.65 + pulse * 0.35);
